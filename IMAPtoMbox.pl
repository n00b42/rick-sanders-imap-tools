...
# $Header: /mhub4/sources/imap-tools/IMAPtoMbox.pl,v 1.17 2015/12/01 06:51:14 rick Exp $
...
#################################################################
   ...
   login($sourceUser,$sourcePwd, $dst, $srcMethod) or exit;
...
sub usage {
   ...
   print STDOUT "    -i host/user/password/CRAM-MD5 to use CRAM-MD5 login method\n";
   exit;
...
sub processArgs {
   ...
   ($sourceHost,$sourceUser,$sourcePwd,$srcMethod) = split(/\//, $opt_i);
   $mbxList  = $opt_M;
   $logfile  = $opt_L;
   $dir      = $opt_m;
   $owner    = $opt_o;
...
my $conn = shift;
   ...
   while (1) {
        ...
        if ( $loops++ > 9 ) {
           Log("No response after 9 retries, exit");
           exit;
        }
