...
# $Header: /mhub4/sources/imap-tools/maildir_to_imap.pl,v 1.11 2015/12/04 22:56:43 rick Exp $
...
sub usage {
   ...
   print "To login with CRAM-MD5 add CRAM-MD5 like this:  -D imaphost/CRAM-MD5\n";
   exit;
...
my $dow;
   ...
   ($dom) = split(/-/, $$date );
   if ( length( $dom ) == 1 ) {
      #  Pad the day to two digits (Zimbra requires it)
      $$date = '0' . $$date;
   }
